...
export default function Chat() {
  ...
  useEffect(() => {
    if (account === undefined) {
        ...
        const characterData = likedList.map(id => datas.characters.find(character => character.id === id));
        if (characterData) {
  ...
  function Choice(){
    return (
			<div className="banner item max-w-[600px] min-h-[250px]">
				<div className="banner-text-area">
					<h3>캐릭터를<br /> '좋아요' 해주세요</h3>
					<p className="banner-text">캐릭터의 별표를<br />  터치하면 해주세요</p>
				</div>
	...
	return (
		<>
			<h2 className="list-title">좋아요 한 캐릭터</h2>
      {account === 'undefined' ? <Choice /> : <CharacterList data={characterData} /> }
