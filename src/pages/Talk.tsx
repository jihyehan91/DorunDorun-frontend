...
function Talk() {
  ...
  async function getMissions() {
    try {
      const response = await axios.get(
        "https://43.203.227.36.sslip.io/server/missions"
      );
  ...
  const backHandler = (talkMessages) => {
    if (
      ...
      !isFinish &&
      !window.confirm("대화창을 나가면 내역은 저장되지 않습니다.")
    )
      return;
    window.history.back();
  };
  ...
  const finishChat = async () => {
    ...
    if (
      todayMissionCount < 3 &&
      !window.confirm(
        "오늘의 학습 미션을 달성하지 못하였습니다. 그만하시겠습니까?"
      )
    )
      return;
    try {
    ...
    <>
      <div className="list-talk bg-cover bg-no-repeat bg-center ">
        ...
        <div ref={innerRef} className="inner">
          <div
            className="bg-char"
            style={{ backgroundImage: `url(/bg_${bgNum}.jpg)` }}
          ></div>
